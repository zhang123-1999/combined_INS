...
EXP_ID_DEFAULT = "EXP-20260319-data2-turn-window-lgx-from-y-gain-scale-r1"
...
OUTPUT_DIR_DEFAULT = Path("output/data2_turn_window_lgx_from_y_gain_scale_probe")
...
def variant_specs() -> list[VariantSpec]:
    return [
        VariantSpec("baseline_joint", "baseline_joint", 0, 1.0, 1.0, "joint baseline reference"),
        VariantSpec("joint_pos_gain_0p5_ref", "joint_pos_gain_0p5_ref", 1, 0.5, 1.0, "current best balanced reference"),
        VariantSpec(
            "joint_pos_gain_0p5_lgx_from_y_0p75",
            "joint_pos_gain_0p5_lgx_from_y_0p75",
            2,
            0.5,
            0.75,
            "retain pos_gain=0.5, suppress only K(lgx, meas_y) to 75%",
        ),
        VariantSpec(
            "joint_pos_gain_0p5_lgx_from_y_0p5",
            "joint_pos_gain_0p5_lgx_from_y_0p5",
            3,
            0.5,
            0.5,
            "retain pos_gain=0.5, suppress only K(lgx, meas_y) to 50%",
        ),
        VariantSpec(
            "joint_pos_gain_0p5_lgx_from_y_0p25",
            "joint_pos_gain_0p5_lgx_from_y_0p25",
            4,
            0.5,
            0.25,
            "retain pos_gain=0.5, suppress only K(lgx, meas_y) to 25%",
        ),
        VariantSpec(
            "joint_pos_gain_0p5_lgx_from_y_0p0",
            "joint_pos_gain_0p5_lgx_from_y_0p0",
            5,
            0.5,
            0.0,
            "retain pos_gain=0.5, completely remove K(lgx, meas_y)",
        ),
    ]
...
def write_summary(output_path: Path, case_meta_rows: list[dict[str, Any]], case_summary_df: pd.DataFrame) -> None:
    merged = case_summary_df.merge(
        pd.DataFrame(case_meta_rows)[
            ["case_id", "label", "position_gain_scale", "lgx_from_y_gain_scale", "variant_description"]
        ],
        on=["case_id", "label"],
        how="left",
    )
    lines: list[str] = []
    lines.append("# data2 turn-window lgx-from-y gain-scale probe")
    lines.append("")
    ...
    for row in merged.itertuples(index=False):
        lines.append(
            ...
            f"`lever_share_x/y={row.mean_lever_share_x:.4f}/{row.mean_lever_share_y:.4f}`, "
            f"`pos_share_x/y={row.mean_pos_share_x:.4f}/{row.mean_pos_share_y:.4f}`, "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Probe selective suppression of K(lgx, meas_y) in turn windows.")
