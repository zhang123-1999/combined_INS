...
EXP_ID_DEFAULT = "EXP-20260325-data2-staged-g5-mounting-zero-init-q-sweep-r2"
OUTPUT_DIR_DEFAULT = Path("output/data2_staged_g5_mounting_zero_init_q_sweep_r2_20260325")
...
PHASE2_MOUNTING_COV_SEED_DURATION_DEFAULT = 0.02
G5_EXTRINSIC_NOISE_SCALE = 0.01
G5_ODO_NHC_R_SCALE_REL_TO_G5 = 6.0
G5_ODO_NHC_MEASUREMENT_NOISE_SCALE = 5.0 * G5_ODO_NHC_R_SCALE_REL_TO_G5
MOUNTING_INIT_STD_DEG = 3.0
...
class CaseSpec:
    ...
    color: str
    description: str
    mounting_q_scale_rel_to_staged_6x: float
...
CASE_SPECS: tuple[CaseSpec, ...] = (
    CaseSpec(
        case_id="staged_g5_mounting_zero_init_q_0p5x",
        label="staged G5 mounting Q 0.5x",
        color="#e45756",
        description="Staged G5 6x baseline with zero-init mounting and 0.5x mounting process noise.",
        mounting_q_scale_rel_to_staged_6x=0.5,
    ),
    CaseSpec(
        case_id="staged_g5_mounting_zero_init_q_1x",
        label="staged G5 mounting Q 1x",
        color="#72b7b2",
        description="Staged G5 6x baseline with zero-init mounting and nominal staged-6x mounting process noise.",
        mounting_q_scale_rel_to_staged_6x=1.0,
    ),
    CaseSpec(
        case_id="staged_g5_mounting_zero_init_q_2x",
        label="staged G5 mounting Q 2x",
        color="#54a24b",
        description="Staged G5 6x baseline with zero-init mounting and 2x mounting process noise.",
        mounting_q_scale_rel_to_staged_6x=2.0,
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the data2 staged G5 mounting zero-init Q sweep with periodic GNSS outage."
    )
    ...
    parser.add_argument(
        "--phase2-mounting-cov-seed-duration",
        type=float,
        default=PHASE2_MOUNTING_COV_SEED_DURATION_DEFAULT,
    )
...
def large_mounting_variance() -> float:
    return float(math.radians(MOUNTING_INIT_STD_DEG) ** 2)
...
def build_runtime_phases(
    ...
    final_time: float,
    phase2_mounting_cov_seed_duration: float,
) -> list[dict[str, Any]]:
    ...
    phase2_seed_end_time = min(
        ...
        float(phase1_end_time) + max(0.0, float(phase2_mounting_cov_seed_duration)),
    )
    phase2_seed = copy.deepcopy(phase2)
    phase2_seed["name"] = "phase2_mounting_cov_seed"
...
def build_case_config(
    ...
    mounting_q_scale = G5_EXTRINSIC_NOISE_SCALE * spec.mounting_q_scale_rel_to_staged_6x
    scale_noise_fields(
        noise_cfg,
        base_noise,
    ...
    fusion["runtime_phases"] = build_runtime_phases(
        ...
        phase2_end_time,
        final_time,
        args.phase2_mounting_cov_seed_duration,
    )
    ...
    metadata: dict[str, Any] = {
        ...
        "noise_scales": {
            ...
            "mounting_q_scale_rel_to_base_q": mounting_q_scale,
            "mounting_q_scale_rel_to_staged_6x": spec.mounting_q_scale_rel_to_staged_6x,
        ...
        "phase2_main_start_time": float(phase1_end_time + max(0.0, args.phase2_mounting_cov_seed_duration)),
        "phase2_end_time": float(phase2_end_time),
        "phase2_mounting_cov_seed_duration_s": float(args.phase2_mounting_cov_seed_duration),
...
def write_summary(
    ...
    overall_rows = [
        [
            str(row["case_id"]),
            format_metric(row.get("mounting_q_scale_rel_to_staged_6x")),
    ...
    lines = [
        "# data2 staged G5 mounting zero-init Q sweep summary",
        "",
        ...
        (
            f"- mounting init: `roll0=pitch0=yaw0=0 deg`, "
            f"`total mounting base=[0,0,0] deg`, "
            f"`P0_diag[22:24]={large_mounting_variance():.12g} rad^2`, "
            f"`std_mounting_roll/pitch/yaw={MOUNTING_INIT_STD_DEG:.1f} deg`"
        ),
        (
            f"- phase2 mounting covariance seed: "
            f"`duration={manifest['experiment_controls']['phase2_mounting_cov_seed_duration_s']:.3f}s`, "
...
def main() -> None:
    ...
    for spec in selected_specs:
        ...
        metrics_row["mounting_q_scale_rel_to_staged_6x"] = spec.mounting_q_scale_rel_to_staged_6x
        metrics_row["mounting_q_scale_rel_to_base_q"] = G5_EXTRINSIC_NOISE_SCALE * spec.mounting_q_scale_rel_to_staged_6x
    ...
    plot_state_grid(
        ...
        all_states_path,
        "Staged G5 mounting zero-init Q sweep: all-state overview",
        phase1_end_time,
        phase2_end_time,
        gnss_off_windows,
    ...
    plot_state_grid(
        ...
        KEY_COUPLING_STATES,
        key_states_path,
        "Staged G5 mounting zero-init Q sweep: key coupling states",
        phase1_end_time,
        phase2_end_time,
        gnss_off_windows,
        truth_keys_to_hide=plot_config.truth_keys_to_hide,
    ...
    for group_spec in plot_config.group_specs:
        ...
        plot_state_grid(
            ...
            group_path,
            f"Staged G5 mounting zero-init Q sweep: {selected_group.title}",
            phase1_end_time,
            phase2_end_time,
            gnss_off_windows,
            plot_mode=plot_mode,
