...
EXP_ID_DEFAULT = "EXP-20260318-data2-turn-window-variant-ab-r1"
...
OUTPUT_DIR_DEFAULT = Path("output/data2_turn_window_variant_ab_probe")
...
def variant_specs() -> list[VariantSpec]:
    true_iekf_mutations = {
        ("fusion", "fej", "enable"): True,
        ("fusion", "fej", "true_iekf_mode"): True,
        ("fusion", "fej", "ri_gnss_pos_use_p_ned_local"): True,
        ("fusion", "fej", "ri_vel_gyro_noise_mode"): 1,
        ("fusion", "fej", "ri_inject_pos_inverse"): True,
    }
    return [
        VariantSpec(
            case_id="baseline",
            ...
            order_idx=0,
            mutations={},
            description="Pure INS/GNSS truth-modeled baseline (ESKF).",
        ),
        VariantSpec(
            case_id="standard_reset_gamma",
            label="eskf_standard_reset_gamma",
            order_idx=1,
            mutations={
                ("fusion", "fej", "debug_enable_standard_reset_gamma"): True,
            },
            description="ESKF baseline with debug standard reset gamma enabled.",
        ),
        VariantSpec(
            case_id="true_iekf",
            label="true_iekf",
            order_idx=2,
            mutations=true_iekf_mutations,
            description="Switch the same pure INS/GNSS case to true_iekf mode.",
        ),
        VariantSpec(
            case_id="position_anchor_diag",
            label="eskf_position_anchor_diag",
            order_idx=3,
            mutations={
                ("fusion", "init", "runtime_truth_anchor_pva"): True,
                ("fusion", "init", "runtime_truth_anchor_gnss_only"): True,
            ...
            description="Diagnostic-only position anchor at GNSS epochs to approximate a position freeze/constraint.",
        ),
    ]
...
def write_summary(output_path: Path, case_meta_rows: list[dict[str, Any]], case_summary_df: pd.DataFrame) -> None:
    merged = case_summary_df.merge(
        pd.DataFrame(case_meta_rows)[["case_id", "label", "variant_description"]],
        on=["case_id", "label"],
        how="left",
    )
    lines: list[str] = []
    lines.append("# data2 turn-window variant A/B probe")
    lines.append("")
    lines.append("## Variants")
    for row in merged.itertuples(index=False):
        lines.append(f"- `{row.case_id}`: {row.variant_description}")
    lines.append("")
    lines.append("## Case summary")
    for row in merged.itertuples(index=False):
        lines.append(
            f"- `{row.case_id}`: `lever_share_x/y/z={row.mean_lever_share_x:.4f}/{row.mean_lever_share_y:.4f}/{row.mean_lever_share_z:.4f}`, "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run turn-window probe on focused A/B variants.")
...
def main() -> int:
    ...
    for spec in variant_specs():
        ...
        per_update_df, window_summary_df, case_summary_df = analyze_case_updates(
            ...
            truth_lever=pd.Series(truth_lever, dtype=float).to_numpy(),
        )
    ...
    manifest = {
        ...
        "truth_lever_internal_m": truth_lever,
        "variants": [
            {
                "case_id": spec.case_id,
                "label": spec.label,
                "description": spec.description,
                "mutations": {".".join(path): value for path, value in spec.mutations.items()},
            }
            for spec in variant_specs()
        ],
        "artifacts": {
