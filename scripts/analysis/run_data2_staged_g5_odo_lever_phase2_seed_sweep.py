...
EXP_ID_DEFAULT = "EXP-20260326-data2-staged-g5-odo-lever-phase2-seed-sweep-r2"
OUTPUT_DIR_DEFAULT = Path("output/data2_staged_g5_odo_lever_phase2_seed_sweep_r2_20260326")
...
class CaseSpec:
    ...
    color: str
    description: str
    odo_lever_phase2_seed_scale_rel_to_base_init: float
...
CASE_SPECS: tuple[CaseSpec, ...] = (
    CaseSpec(
        case_id="staged_g5_odo_lever_phase2_seed_0p5x",
        label="staged G5 odo_lever seed 0.5x",
        color="#e45756",
        description="Odo-scale-2x baseline with odo_lever zero-init and phase2 lever covariance seed at 0.5x of base init std.",
        odo_lever_phase2_seed_scale_rel_to_base_init=0.5,
    ),
    CaseSpec(
        case_id="staged_g5_odo_lever_phase2_seed_1x",
        label="staged G5 odo_lever seed 1x",
        color="#72b7b2",
        description="Odo-scale-2x baseline with odo_lever zero-init and phase2 lever covariance seed at 1x of base init std.",
        odo_lever_phase2_seed_scale_rel_to_base_init=1.0,
    ),
    CaseSpec(
        case_id="staged_g5_odo_lever_phase2_seed_2x",
        label="staged G5 odo_lever seed 2x",
        color="#54a24b",
        description="Odo-scale-2x baseline with odo_lever zero-init and phase2 lever covariance seed at 2x of base init std.",
        odo_lever_phase2_seed_scale_rel_to_base_init=2.0,
    ),
    CaseSpec(
        case_id="staged_g5_odo_lever_phase2_seed_4x",
        label="staged G5 odo_lever seed 4x",
        color="#4c78a8",
        description="Odo-scale-2x baseline with odo_lever zero-init and phase2 lever covariance seed at 4x of base init std.",
        odo_lever_phase2_seed_scale_rel_to_base_init=4.0,
    ),
    CaseSpec(
        case_id="staged_g5_odo_lever_phase2_seed_8x",
        label="staged G5 odo_lever seed 8x",
        color="#b279a2",
        description="Odo-scale-2x baseline with odo_lever zero-init and phase2 lever covariance seed at 8x of base init std.",
        odo_lever_phase2_seed_scale_rel_to_base_init=8.0,
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the data2 staged G5 odo_lever phase2 covariance-seed sweep on top of the odo_scale-2x baseline."
    )
    ...
    parser.add_argument(
        "--phase2-cov-seed-duration",
        type=float,
        default=PHASE2_COV_SEED_DURATION_DEFAULT,
    )
...
def build_case_config(
    ...
    scale_noise_fields(
        noise_cfg,
        base_noise,
        ["sigma_lever_arm", "sigma_gnss_lever_arm"],
        G5_EXTRINSIC_NOISE_SCALE,
    )
    ...
    phase2_odo_scale_seed_std = (
        float(base_init["std_odo_scale"]) * ODO_SCALE_PHASE2_SEED_SCALE_REL_TO_BASE_INIT_FIXED
    )
    phase2_odo_scale_seed_var = phase2_odo_scale_seed_std * phase2_odo_scale_seed_std
    phase2_lever_seed_std_vec = [
        float(value) * spec.odo_lever_phase2_seed_scale_rel_to_base_init for value in base_init["std_lever_arm"]
    ]
    ...
    metadata: dict[str, Any] = {
        ...
        "odo_lever_phase2_seed": {
            ...
            "scale_rel_to_base_init": float(spec.odo_lever_phase2_seed_scale_rel_to_base_init),
        },
        "noise_scales": {
            "lever_q_scale_rel_to_base_q": G5_EXTRINSIC_NOISE_SCALE,
            "mounting_q_scale_rel_to_base_q": G5_EXTRINSIC_NOISE_SCALE * MOUNTING_Q_SCALE_REL_TO_STAGED_6X_FIXED,
            "mounting_q_scale_rel_to_staged_6x": float(MOUNTING_Q_SCALE_REL_TO_STAGED_6X_FIXED),
...
def write_summary(
    ...
    lines = [
        "# data2 staged G5 odo_lever phase2 covariance-seed sweep summary",
        "",
        ...
        (
            f"- phase2 covariance seed: `duration={manifest['experiment_controls']['phase2_cov_seed_duration_s']:.3f}s`, "
...
def main() -> None:
    ...
    for spec in selected_specs:
        ...
        metrics_row["odo_lever_phase2_seed_scale_rel_to_base_init"] = spec.odo_lever_phase2_seed_scale_rel_to_base_init
        metrics_row["mounting_q_scale_rel_to_staged_6x"] = float(MOUNTING_Q_SCALE_REL_TO_STAGED_6X_FIXED)
        metrics_row["odo_scale_phase2_seed_scale_rel_to_base_init"] = float(
            ODO_SCALE_PHASE2_SEED_SCALE_REL_TO_BASE_INIT_FIXED
        )
    ...
    plot_state_grid(
        ...
        all_states_path,
        "Staged G5 odo_lever phase2 covariance-seed sweep: all-state overview",
        phase1_end_time,
        phase2_end_time,
        gnss_off_windows,
    ...
    plot_state_grid(
        ...
        KEY_COUPLING_STATES,
        key_states_path,
        "Staged G5 odo_lever phase2 covariance-seed sweep: key coupling states",
        phase1_end_time,
        phase2_end_time,
        gnss_off_windows,
        truth_keys_to_hide=plot_config.truth_keys_to_hide,
    ...
    for group_spec in plot_config.group_specs:
        ...
        plot_state_grid(
            ...
            group_path,
            f"Staged G5 odo_lever phase2 covariance-seed sweep: {selected_group.title}",
            phase1_end_time,
            phase2_end_time,
            gnss_off_windows,
            plot_mode=plot_mode,
    ...
    manifest = {
        ...
        "experiment_controls": {
            ...
            "base_odo_lever_seed_var_vec": [float(value) ** 2 for value in base_cfg["fusion"]["init"]["std_lever_arm"]],
        },
    }
