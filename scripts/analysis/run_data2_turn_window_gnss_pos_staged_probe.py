...
EXP_ID_DEFAULT = "EXP-20260318-data2-turn-window-gnsspos-staged-r1"
...
OUTPUT_DIR_DEFAULT = Path("output/data2_turn_window_gnss_pos_staged_probe")
...
def variant_specs() -> list[VariantSpec]:
    return [
        VariantSpec(
            case_id="baseline_joint",
            label="joint_pos_gain_1p0",
            order_idx=0,
            mutations={},
            description="Default single-step GNSS_POS joint update.",
        ),
        VariantSpec(
            case_id="joint_pos_gain_0p5",
            label="joint_pos_gain_0p5",
            order_idx=1,
            mutations={
                ("fusion", "gnss_pos_position_gain_scale"): 0.5,
            },
            description="Single-step GNSS_POS joint update with position gain scale = 0.5.",
        ),
        VariantSpec(
            case_id="staged_nonpos_then_pos",
            label="staged_pos_gain_1p0",
            order_idx=2,
            mutations={
                ("fusion", "gnss_pos_update_mode"): "stage_nonpos_then_pos",
            },
            description="Two-stage GNSS_POS update: non-position states first, then position-only.",
        ),
        VariantSpec(
            case_id="staged_nonpos_then_pos_pos_gain_0p5",
            label="staged_pos_gain_0p5",
            order_idx=3,
            mutations={
                ("fusion", "gnss_pos_update_mode"): "stage_nonpos_then_pos",
                ("fusion", "gnss_pos_position_gain_scale"): 0.5,
            },
            description="Two-stage GNSS_POS update with stage-2 position gain scale = 0.5.",
        ),
    ]
...
def run_variant(spec: VariantSpec, source_cfg_path: Path, case_dir: Path, exe_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "gnss_pos_update_mode": str(cfg["fusion"].get("gnss_pos_update_mode", "joint")),
        "gnss_pos_position_gain_scale": float(cfg["fusion"].get("gnss_pos_position_gain_scale", 1.0)),
...
def write_summary(output_path: Path, case_meta_rows: list[dict[str, Any]], case_summary_df: pd.DataFrame) -> None:
    merged = case_summary_df.merge(
        pd.DataFrame(case_meta_rows)[
            ["case_id", "label", "variant_description", "gnss_pos_update_mode", "gnss_pos_position_gain_scale"]
        ],
        on=["case_id", "label"],
        how="left",
    )
    lines: list[str] = []
    lines.append("# data2 turn-window GNSS_POS staged probe")
    lines.append("")
    lines.append("## Variants")
    for row in merged.itertuples(index=False):
        lines.append(
            f"- `{row.case_id}`: mode=`{row.gnss_pos_update_mode}`, "
            f"`pos_gain_scale={row.gnss_pos_position_gain_scale:.2f}`. {row.variant_description}"
        )
    lines.append("")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run staged GNSS_POS turn-window probe.")
