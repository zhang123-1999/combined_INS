...
EXP_ID_DEFAULT = "EXP-20260326-data2-staged-g5-odo-scale-phase2-seed-sweep-r1"
OUTPUT_DIR_DEFAULT = Path("output/data2_staged_g5_odo_scale_phase2_seed_sweep_r1_20260326")
...
class CaseSpec:
    ...
    color: str
    description: str
    odo_scale_phase2_seed_scale_rel_to_base_init: float
...
CASE_SPECS: tuple[CaseSpec, ...] = (
    CaseSpec(
        case_id="staged_g5_odo_scale_phase2_seed_0p5x",
        label="staged G5 odo_scale seed 0.5x",
        color="#e45756",
        description="Mounting-1x baseline with phase2 odo_scale covariance seed at 0.5x of base init std.",
        odo_scale_phase2_seed_scale_rel_to_base_init=0.5,
    ),
    CaseSpec(
        case_id="staged_g5_odo_scale_phase2_seed_1x",
        label="staged G5 odo_scale seed 1x",
        color="#72b7b2",
        description="Mounting-1x baseline with phase2 odo_scale covariance seed at 1x of base init std.",
        odo_scale_phase2_seed_scale_rel_to_base_init=1.0,
    ),
    CaseSpec(
        case_id="staged_g5_odo_scale_phase2_seed_2x",
        label="staged G5 odo_scale seed 2x",
        color="#54a24b",
        description="Mounting-1x baseline with phase2 odo_scale covariance seed at 2x of base init std.",
        odo_scale_phase2_seed_scale_rel_to_base_init=2.0,
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the data2 staged G5 odo_scale phase2 covariance-seed sweep on top of mounting-1x baseline."
    )
...
def build_case_config(
    ...
    phase2_odo_scale_seed_std = float(base_init["std_odo_scale"]) * spec.odo_scale_phase2_seed_scale_rel_to_base_init
    ...
    metadata: dict[str, Any] = {
        ...
        "odo_scale_phase2_seed": {
            ...
            "scale_rel_to_base_init": float(spec.odo_scale_phase2_seed_scale_rel_to_base_init),
        },
        "noise_scales": {
...
def write_summary(
    ...
    overall_rows = [
        [
            str(row["case_id"]),
            format_metric(row.get("odo_scale_phase2_seed_scale_rel_to_base_init")),
    ...
    lines = [
        "# data2 staged G5 odo_scale phase2 covariance-seed sweep summary",
        "",
        ...
        (
            f"- fixed mounting baseline: `Q_rel_staged_6x={MOUNTING_Q_SCALE_REL_TO_STAGED_6X_FIXED:.1f}x`, "
            f"`roll0=pitch0=yaw0=0 deg`, `P0_diag[22:24]={large_mounting_variance():.12g} rad^2`"
        ),
        (
            ...
            f"`odo_scale_seed_var={manifest['experiment_controls']['base_odo_scale_seed_var']:.12g}` for 1x case"
        ),
...
def main() -> None:
    ...
    for spec in selected_specs:
        ...
        metrics_row["odo_scale_phase2_seed_scale_rel_to_base_init"] = spec.odo_scale_phase2_seed_scale_rel_to_base_init
    ...
    plot_state_grid(
        ...
        all_states_path,
        "Staged G5 odo_scale phase2 covariance-seed sweep: all-state overview",
        phase1_end_time,
        phase2_end_time,
        gnss_off_windows,
    ...
    plot_state_grid(
        ...
        KEY_COUPLING_STATES,
        key_states_path,
        "Staged G5 odo_scale phase2 covariance-seed sweep: key coupling states",
        phase1_end_time,
        phase2_end_time,
        gnss_off_windows,
        truth_keys_to_hide=plot_config.truth_keys_to_hide,
    ...
    for group_spec in plot_config.group_specs:
        ...
        plot_state_grid(
            ...
            group_path,
            f"Staged G5 odo_scale phase2 covariance-seed sweep: {selected_group.title}",
            phase1_end_time,
            phase2_end_time,
            gnss_off_windows,
            plot_mode=plot_mode,
    ...
    manifest = {
        ...
        "experiment_controls": {
            ...
            "phase2_cov_seed_duration_s": float(args.phase2_cov_seed_duration),
            "phase2_mounting_cov_seed_floor_deg": float(MOUNTING_INIT_STD_DEG),
            "base_odo_scale_seed_var": float(base_cfg["fusion"]["init"]["std_odo_scale"]) ** 2,
        },
    }
