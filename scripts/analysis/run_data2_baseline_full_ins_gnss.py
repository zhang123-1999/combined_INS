...
EXP_ID_DEFAULT = "EXP-20260327-data2-baseline-full-ins-gnss-r1"
OUTPUT_DIR_DEFAULT = Path("output/data2_baseline_full_ins_gnss")
BASE_CONFIG_DEFAULT = Path("config_data2_baseline_eskf.yaml")
SOLVER_DEFAULT = Path("build/Release/eskf_fusion.exe")
CASE_ID_DEFAULT = "data2_baseline_full_ins_gnss"
CASE_LABEL_DEFAULT = "data2 baseline full INS/GNSS"
...
def apply_process_noise_scale(cfg: dict[str, Any], process_noise_scale_rel: float) -> None:
    process_noise_scale_rel = float(process_noise_scale_rel)
    if process_noise_scale_rel <= 0.0:
        raise ValueError("process_noise_scale_rel must be positive")

    fusion = cfg.setdefault("fusion", {})
    noise = fusion.setdefault("noise", {})
    for key in ["sigma_ba", "sigma_bg", "sigma_sg", "sigma_sa"]:
        if key not in noise:
            raise KeyError(f"missing required noise field for process scaling: {key}")
        scaled = float(noise[key]) * process_noise_scale_rel
        noise[key] = scaled
        noise[f"{key}_vec"] = [scaled, scaled, scaled]
...
def extract_run_metadata(
    ...
    metadata = {
        ...
        "full_window": [float(fusion["starttime"]), float(fusion["finaltime"])],
...
def build_run_config(
    ...
    output_dir: Path,
    case_id: str = CASE_ID_DEFAULT,
    case_label: str = CASE_LABEL_DEFAULT,
    process_noise_scale_rel: float = 1.0,
        ...
        "enabled_windows": [
            {
                ...
                "end_time": float(fusion["finaltime"]),
            }
        ],
    }
    apply_process_noise_scale(cfg, process_noise_scale_rel)
    if fix_gnss_lever_truth:
        apply_fixed_gnss_lever_truth_contract(cfg)
    return cfg, extract_run_metadata(
        cfg,
        case_id,
        case_label,
        process_noise_scale_rel,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the fixed-solver data2 baseline as a full-window INS/GNSS baseline without ODO/NHC or runtime phases."
    )
    ...
    parser.add_argument("--case-label", default=CASE_LABEL_DEFAULT)
    parser.add_argument("--process-noise-scale-rel", type=float, default=1.0)
...
def run_case(
    ...
    if not root_diag.exists():
        raise RuntimeError("missing DIAG.txt after full INS/GNSS baseline run")
...
def write_summary(
    ...
    lines = [
        "# data2 full-window INS/GNSS baseline summary",
        "",
        f"- exp_id: `{manifest['exp_id']}`",
        f"- base_config: `{manifest['base_config']}`",
        f"- output_dir: `{manifest['output_dir']}`",
        f"- case_id: `{manifest['case_id']}`",
        f"- case_label: `{manifest['case_label']}`",
        f"- process_noise_scale_rel: `{manifest['process_noise_scale_rel']}`",
        f"- fix_gnss_lever_truth: `{manifest['fix_gnss_lever_truth']}`",
        f"- full window: `{manifest['full_window']}`",
        f"- GNSS enabled windows: `{manifest['gnss_on_windows']}`",
...
def main() -> None:
    ...
    cfg, metadata = build_run_config(
        ...
        process_noise_scale_rel=args.process_noise_scale_rel,
    ...
    phase_windows = [("full_window_ins_gnss", *metadata["full_window"])]
    ...
    plot_case = PlotCase(case_id=args.case_id, label=args.case_label, color="#1f77b4")
    ...
    plot_boundary_time = float(metadata["full_window"][1])
    ...
    plot_state_grid(
        ...
        overview_path,
        "data2 full-window INS/GNSS all-state overview",
        plot_boundary_time,
        plot_boundary_time,
        gnss_off_windows,
    ...
    plot_state_grid(
        ...
        KEY_COUPLING_STATES,
        key_path,
        "data2 full-window INS/GNSS key coupling states",
        plot_boundary_time,
        plot_boundary_time,
        gnss_off_windows,
        truth_keys_to_hide=plot_config.truth_keys_to_hide,
    ...
    for group_spec in plot_config.group_specs:
        ...
        plot_state_grid(
            ...
            group_path,
            f"data2 full-window INS/GNSS {selected_group.title}",
            plot_boundary_time,
            plot_boundary_time,
            gnss_off_windows,
            plot_mode=plot_mode,
    ...
    manifest = {
        ...
        "case_label": args.case_label,
        "process_noise_scale_rel": args.process_noise_scale_rel,
        "fix_gnss_lever_truth": metadata["fix_gnss_lever_truth"],
        ...
        "full_window": metadata["full_window"],
