...
EXP_ID_DEFAULT = "EXP-20260319-data2-turn-window-lgy-from-y-gain-scale-r1"
...
OUTPUT_DIR_DEFAULT = Path("output/data2_turn_window_lgy_from_y_gain_scale_probe")
EXE_DEFAULT = Path("build/Release/eskf_fusion.exe")
POSITIVE_TURN_THRESHOLD_DEG_S = 8.0
POSITIVE_TURN_POSITION_GAIN_SCALE = 0.25
...
class VariantSpec:
    ...
    lgx_from_y_gain_scale: float
    lgy_from_y_gain_scale: float
    turn_rate_threshold_deg_s: float
    positive_turn_position_gain_scale: float
    negative_turn_position_gain_scale: float
    positive_turn_lgy_from_y_gain_scale: float
    negative_turn_lgy_from_y_gain_scale: float
...
def variant_specs() -> list[VariantSpec]:
    return [
        VariantSpec(
            "joint_pos_gain_0p5_lgx_from_y_0p25_ref",
            "joint_pos_gain_0p5_lgx_from_y_0p25_ref",
            0,
            0.5,
            0.25,
            ...
            -1.0,
            -1.0,
            "x reference family only: pos_gain=0.5 with lgx<-y selective suppression 0.25",
        ),
        VariantSpec(
            ...
            0.25,
            1.0,
            POSITIVE_TURN_THRESHOLD_DEG_S,
            POSITIVE_TURN_POSITION_GAIN_SCALE,
            -1.0,
            -1.0,
            -1.0,
            "current best combined reference: add positive-turn position suppression on top of x reference family",
        ),
        VariantSpec(
            ...
            0.75,
            -1.0,
            "retain best combined reference and suppress positive-turn K(lgy, meas_y) to 75%",
        ),
        VariantSpec(
            "joint_pos_gain_0p5_lgx_from_y_0p25_pos_turn_0p25_lgy_from_y_0p5",
            "joint_pos_gain_0p5_lgx_from_y_0p25_pos_turn_0p25_lgy_from_y_0p5",
            3,
            0.5,
            0.25,
            ...
            0.5,
            -1.0,
            "retain best combined reference and suppress positive-turn K(lgy, meas_y) to 50%",
        ),
        VariantSpec(
            "joint_pos_gain_0p5_lgx_from_y_0p25_pos_turn_0p25_lgy_from_y_0p25",
            "joint_pos_gain_0p5_lgx_from_y_0p25_pos_turn_0p25_lgy_from_y_0p25",
            4,
            0.5,
            0.25,
            ...
            0.25,
            -1.0,
            "retain best combined reference and suppress positive-turn K(lgy, meas_y) to 25%",
        ),
        VariantSpec(
            ...
            0.0,
            -1.0,
            "retain best combined reference and fully remove positive-turn K(lgy, meas_y)",
        ),
    ]
...
def build_variant_config(spec: VariantSpec, source_cfg_path: Path, case_dir: Path) -> tuple[dict[str, Any], Path]:
    ...
    fusion["gnss_pos_lgx_from_y_gain_scale"] = float(spec.lgx_from_y_gain_scale)
    fusion["gnss_pos_lgy_from_y_gain_scale"] = float(spec.lgy_from_y_gain_scale)
    fusion["gnss_pos_turn_rate_threshold_deg_s"] = float(spec.turn_rate_threshold_deg_s)
    fusion["gnss_pos_positive_turn_position_gain_scale"] = float(spec.positive_turn_position_gain_scale)
    fusion["gnss_pos_negative_turn_position_gain_scale"] = float(spec.negative_turn_position_gain_scale)
    fusion["gnss_pos_positive_turn_lgy_from_y_gain_scale"] = float(spec.positive_turn_lgy_from_y_gain_scale)
    fusion["gnss_pos_negative_turn_lgy_from_y_gain_scale"] = float(spec.negative_turn_lgy_from_y_gain_scale)
...
def run_variant(spec: VariantSpec, source_cfg_path: Path, case_dir: Path, exe_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "lgx_from_y_gain_scale": float(spec.lgx_from_y_gain_scale),
        "lgy_from_y_gain_scale": float(spec.lgy_from_y_gain_scale),
        "turn_rate_threshold_deg_s": float(spec.turn_rate_threshold_deg_s),
        "positive_turn_position_gain_scale": float(spec.positive_turn_position_gain_scale),
        "negative_turn_position_gain_scale": float(spec.negative_turn_position_gain_scale),
        "positive_turn_lgy_from_y_gain_scale": float(spec.positive_turn_lgy_from_y_gain_scale),
        "negative_turn_lgy_from_y_gain_scale": float(spec.negative_turn_lgy_from_y_gain_scale),
...
def write_summary(output_path: Path, case_meta_rows: list[dict[str, Any]], case_summary_df: pd.DataFrame) -> None:
    merged = case_summary_df.merge(
        pd.DataFrame(case_meta_rows)[
            [
                ...
                "positive_turn_position_gain_scale",
                "positive_turn_lgy_from_y_gain_scale",
                "variant_description",
            ]
        ],
        on=["case_id", "label"],
    ...
    lines: list[str] = []
    lines.append("# data2 turn-window lgy-from-y gain-scale probe")
    lines.append("")
    lines.append("## Case summary")
    for row in merged.itertuples(index=False):
        lines.append(
            f"- `{row.case_id}` (`pos_gain_scale={row.position_gain_scale:.2f}`, "
            f"`lgx_from_y_gain_scale={row.lgx_from_y_gain_scale:.2f}`, "
            f"`lgy_from_y_gain_scale={row.lgy_from_y_gain_scale:.2f}`, "
            f"`turn_thresh_deg_s={row.turn_rate_threshold_deg_s:.1f}`, "
            f"`pos_turn_pos_gain={row.positive_turn_position_gain_scale:.2f}`, "
            f"`pos_turn_lgy_from_y_gain={row.positive_turn_lgy_from_y_gain_scale:.2f}`): "
            f"`lever_x/y={row.mean_lever_share_x:.4f}/{row.mean_lever_share_y:.4f}`, "
            f"`pos_x/y={row.mean_pos_share_x:.4f}/{row.mean_pos_share_y:.4f}`, "
            f"`lever_x/y_error_improve={row.mean_lever_x_error_improve_m:.6f}/{row.mean_lever_y_error_improve_m:.6f}` m. "
            f"{row.variant_description}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Probe positive-turn selective suppression of K(lgy, meas_y) on top of the current best combined family."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Only run the selected case id(s). Can be passed multiple times.",
    )
    return parser
...
def main() -> int:
    ...
    specs = [spec for spec in variant_specs() if selected_case_ids is None or spec.case_id in selected_case_ids]
    if not specs:
        raise RuntimeError("no variant selected after applying --case-id filter")
    ...
    case_summary_df = (
        pd.concat(all_case_summaries, ignore_index=True).sort_values(by="r_scale").reset_index(drop=True)
    )
