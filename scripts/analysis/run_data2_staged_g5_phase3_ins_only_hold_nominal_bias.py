...
EXP_ID_DEFAULT = "EXP-20260326-data2-staged-g5-phase3-ins-only-hold-nominal-bias-r1"
OUTPUT_DIR_DEFAULT = Path("output/data2_staged_g5_phase3_ins_only_hold_nominal_bias_r1_20260326")
BASE_CONFIG_DEFAULT = Path("config_data2_research_seed_eskf.yaml")
SOLVER_DEFAULT = Path("build/Release/eskf_fusion.exe")
CASE_ID_DEFAULT = "staged_g5_odo_nhc_noise_6x_phase3_ins_only_hold_nominal_ba_bg"
...
CASE_SPEC = CaseSpec(
    ...
    label="staged G5 6x phase3 INS-only hold nominal ba/bg",
    color="#c44e52",
    description=(
        "Three-phase staged G5 control matching the phase3 INS-only outage case, "
        "but phase3 keeps nominal ba/bg fixed instead of applying GM exponential decay."
    ),
    ...
    phase3_constraint_mode="ins_gnss_outage_control_hold_nominal_ba_bg",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the staged G5 6x phase3 INS-only control with phase3 nominal ba/bg decay disabled."
        )
    )
...
def build_case_config(
    ...
    phase3_noise = runtime_phases[2].setdefault("noise", {})
    phase3_noise["disable_nominal_ba_bg_decay"] = True
    metadata["case_id"] = CASE_ID_DEFAULT
    metadata["reference_case_id"] = REFERENCE_CASE_ID
    metadata["nominal_bias_decay_override"] = {"phase3_disable_nominal_ba_bg_decay": True}
    return cfg, metadata
...
def run_case(cfg_path: Path, case_dir: Path, exe_path: Path) -> dict[str, Any]:
    ...
    if not root_diag.exists():
        raise RuntimeError("missing DIAG.txt after staged G5 hold-nominal-bias run")
...
def write_summary(
    ...
    lines = [
        "# data2 staged G5 phase3 INS-only hold nominal ba/bg summary",
        "",
...
def main() -> None:
    ...
    phase_windows = [
        ...
        ("phase3_periodic_gnss_outage_hold_nominal_ba_bg", metadata["phase2_end_time"], final_time),
    ]
    ...
    plot_state_grid(
        ...
        overview_path,
        "data2 staged G5 hold nominal ba/bg all-state overview",
    ...
    plot_state_grid(
        ...
        KEY_COUPLING_STATES,
        key_path,
        "data2 staged G5 hold nominal ba/bg key coupling states",
        metadata["phase1_end_time"],
        metadata["phase2_end_time"],
        gnss_off_windows,
        truth_keys_to_hide=plot_config.truth_keys_to_hide,
    ...
    for group_spec in plot_config.group_specs:
        ...
        plot_state_grid(
            ...
            group_path,
            f"data2 staged G5 hold nominal ba/bg {selected_group.title}",
            metadata["phase1_end_time"],
            metadata["phase2_end_time"],
            gnss_off_windows,
            plot_mode=plot_mode,
