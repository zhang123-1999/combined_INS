...
def build_run_config(base_cfg: dict[str, Any], output_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    fusion["inekf"] = {"enable": True}
    return cfg, extract_run_metadata(cfg)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the pure INS/GNSS data2 InEKF outage 60on100off experiment using the ESKF best configuration as the shared noise baseline."
    )
...
def run_case(cfg_path: Path, output_dir: Path, case_dir: Path, exe_path: Path) -> dict[str, Any]:
    ...
    if not root_diag.exists():
        raise RuntimeError("missing DIAG.txt after pure INS/GNSS InEKF outage best run")
    shutil.copy2(root_diag, diag_path)

    nav_metrics, segment_rows = evaluate_navigation_metrics(cfg_path, sol_path)
    row: dict[str, Any] = {
        "case_id": BASELINE_CASE_ID,
        "case_label": "data2 baseline ins gnss InEKF outage 60on100off from ESKF best",
...
def write_summary(
    ...
    lines = [
        "# data2 baseline ins gnss InEKF outage 60on100off from ESKF best summary",
        "",
        f"- exp_id: `{manifest['exp_id']}`",
        f"- base_config: `{manifest['base_config']}`",
        f"- output_dir: `{manifest['output_dir']}`",
        f"- case_id: `{BASELINE_CASE_ID}`",
        "- contract: pure `INS/GNSS`, `ODO/NHC` disabled, GNSS lever fixed to truth, `InEKF` enabled",
        "- noise source: shared with the canonical pure `INS/GNSS` ESKF best config",
        (
...
def main() -> None:
    ...
    plot_case = PlotCase(
        case_id=BASELINE_CASE_ID,
        label="data2 baseline ins gnss InEKF outage 60on100off from ESKF best",
        color="#1f77b4",
    )
    ...
    plot_state_grid(
        ...
        overview_path,
        "data2 baseline ins gnss InEKF outage 60on100off all-state overview",
    ...
    plot_state_grid(
        ...
        KEY_COUPLING_STATES,
        key_path,
        "data2 baseline ins gnss InEKF outage 60on100off key coupling states",
        metadata["phase1_window"][1],
        metadata["phase3_window"][0],
        gnss_off_windows,
        truth_keys_to_hide=plot_config.truth_keys_to_hide,
    ...
    for group_spec in plot_config.group_specs:
        ...
        plot_state_grid(
            ...
            group_path,
            f"data2 baseline ins gnss InEKF outage 60on100off {selected_group.title}",
            metadata["phase1_window"][1],
            metadata["phase3_window"][0],
            gnss_off_windows,
            plot_mode=plot_mode,
